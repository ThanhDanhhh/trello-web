...
function BoardContent({
  ...
  // Điểm va chạm cuối cùng trước đó (xử lý tuật toán phát hiện va chạm)
  ...
  // Khởi tạo Function chung xử lý việc cập nhật lại state trng trường hợp di chuyển Card giữa các Column khác nhau.
  const moveCardBetweenDifferentColumns = (
    ...
    setOrderedColumns(preveColumns => {
      ...
      if (triggerFrom === 'handleDragEnd') {
         ...
         * Lưu ý: Về sau ở học phần nâng cao thì chúng ta đưa dữ liệu từ Board ra ngoài Redux Global Store.
         * và lúc này chúng ta có thể gọi luôn Api ở đây là xong thay vì phải lần lượt gọi ngược lên những component cha phía trên. (Đối với component con nằm
         * càng sâu thì càng khổ.)
         */
        // Phải dùng tới activeDragItemData.comlumnId hoặc tốt nhất là oldColuưmWhenDraggingCard._d (set vào State
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeDragItemType === ACTIVIE_DRAG_ITEM_TYPE.CARD) {
      ...
      // Phải dùng tới activeDragItemData.columnId hoặc oldWhenDraggingCard._id  này vì (set từ bước handleDragStart)
      ...
      if (oldColumnWhenDraggingCard._id !== overColumn._id) {
        ...
        //Vẫn gọi update state ở đây tránh delay hoặc flickẻing giao diện lúc kéo thả cần phải chời gọi API (smaill trick)
        setOrderedColumns(preveColumns => {
          // Clone mảng OderedColumnsState cũ ra một cái mới để xử lý data rồi return - cập nhật lại OderedColumns mới
          const nextColumns = cloneDeep(preveColumns)
        ...
        /**
         * gọi lên prop function moveColumn nằm ở component cha cao nhất (board/._id.jsx)
         * Lưu ý: Về sau ở học phần nâng cao thì chúng ta sữ đưa dữ liệu Board ra ngoài Redux GLobal Stỏe.
  ...
  // Chúng ta sẽ custom lại chiến lược / thuật toán phát hiện va chạm tối ưu cho việc kéo thả card giữa nhiều
  // column
  ...
  const collisionDetectionStrategy = useCallback((args) => {
    ...
    // Tìm các điểm giao nhau, va chạm sẽ trả về một mảng các va chạm ở đây - intersections với con trỏ
    ...
    // Kéo một cái card có image cover lớn và kéo lên trên cùng ra khỏi khu vực kéo thả.
    if (!pointerIntersections?.length) return

    // Thuật toán phát hiện va sẽ trả về một mảng các va chạm ở đây(không caàn bước này nữa)
  ...
  return (
    <DndContext
      ...
      // lúc này nó đang bị conflict giữa card và column), chúng ta sẽ dùng closéCorners thay vì closestCenter
      // https://dóc.dndkit.com/api-documenttation/context-provider/collosion-detection-algorithms
